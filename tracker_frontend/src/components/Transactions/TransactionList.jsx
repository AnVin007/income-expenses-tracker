...
const TransactionList = () => {
  ...
  const handleFilterChange = (event) => {
    ...
    // Set filters in order to use them in API
    setFilters((prev) => ({ ...prev, [name]: value })); //! add to particular previous state by using key value pair
  };
  ...
  const {
    ...
    queryFn: () => listTransactionAPI(filters), //! call use query with passing additional data to API
    ...
    <div className="my-4 p-4 shadow-lg rounded-lg bg-white">
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
        ...
        <input
          type="date"
          name="startDate"
          value={filters.startDate} //! value from filters state
          onChange={handleFilterChange} //! method to handle change of input
        ...
        <input
          type="date"
          name="endDate"
          value={filters.endDate} //! value from filters state
          onChange={handleFilterChange} //! method to handle change of input
          className="p-2 rounded-lg border-gray-300 focus:border-blue-500 focus:ring focus:ring-blue-500 focus:ring-opacity-50"
        />
        {/* Type */}
        <div className="relative">
          <select
            name="type"
            value={filters.type} //! value from filters state
            onChange={handleFilterChange} //! method to handle change of input
